...
function setup() {
  ...
  jungle = createSprite(width/2+100,200,width,height);
  jungle.addImage("jungle",jungleImage);
  jungle.scale=3.4;
  jungle.x = width /2;

  boy = createSprite(width/4-500,200,20,50);
  boy.addAnimation("running",boy_running);
  boy.addAnimation("collided", boy_collided);
  boy.scale = 0.20;
  boy.setCollider("circle",0,0,300)
    
  invisibleGround = createSprite(width/2+100,500,1600,10);
...
applenum=createSprite(camera.position.x+980,90)
applenum.addImage(apple)
applenum.scale=0.09
pineapplenum=createSprite(camera.position.x+980,130)
pineapplenum.addImage(pineapple)
pineapplenum.scale=0.09
basketnum=createSprite(camera.position.x+980,170)
 basketnum.addImage(basketImg)
 basketnum.scale=0.09
 
kiwinum=createSprite(camera.position.x+980,220)
kiwinum.scale=0.2
...
function draw() {
  ...
  if(shrubsGroup.isTouching(boy)){
    ...
    shrubsGroup.destroyEach();
              ...
              if(flag===1){
                if (frameCount % 150 === 0) {
                kiwi=createSprite(camera.position.x+504,480,40,10);
              ...
              if(flag===2){
                ...
                basket=createSprite(camera.position.x+500,480,40,10);
  ...
  text(" = "+ acount, camera.position.x+340,90);
  text(" = "+ bcount, camera.position.x+340,170);
  text(" = "+ kcount, camera.position.x+340,220);
  text(" = "+ pcount, camera.position.x+340,130);
...
function spawnShrubs() {
  ...
  if (frameCount % 150 === 0) {
    ...
    var shrub = createSprite(camera.position.x+500,480,40,10);
...
function spawnObstacles() {
  if(frameCount % 120 === 0) {
    ...
    var obstacle = createSprite(camera.position.x+400,480,40,40);
